...
if (isset($_POST['btnEdit'])) {
    ...
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "UPDATE sub_categories SET `image`='$upload_image' WHERE `id`='$ID'";
        $db->sql($sql);
        ...
        if ($update_result == 1) {
            $error['update_slide'] = " <section class='content-header'><span class='label label-success'>Sub Categories updated Successfully</span></section>";
        } else {
...
$sql_query = "SELECT * FROM `sub_categories` WHERE id = '$ID'";
$db->sql($sql_query);
$res = $db->getResult();
?>
<section class="content-header">
	<h1>
		Edit Sub Categories<small><a href='sub_categories.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Sub Categories</a></small></h1>
